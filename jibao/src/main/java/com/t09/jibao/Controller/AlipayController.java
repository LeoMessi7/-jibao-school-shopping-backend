...
public class AlipayController {
    ...
    @ResponseBody
    public String goAlipay(@RequestParam(value = "amount") String amount)
        ...
        model.setBody("充值");
        AlipayConfig.alipayUid = (Long) request.getSession().getAttribute("uid");
    ...
    public String return_url(HttpServletResponse res, HttpServletRequest req) throws IOException, AlipayApiException {
        ...
        if (verifyResult) {
            //商户订单号
            String out_trade_no = AlipayUtil.getInstance().getByte(req.getParameter("out_trade_no"));
            //支付宝交易号
    ...
    @ResponseBody
    public String notify_url(HttpServletRequest req) throws IOException, AlipayApiException {
        ...
        boolean flag = AlipayUtil.getInstance().rsaCertCheckV1(req);
        if (flag) {
            /**
            ...
            //交易状态
            String tradeStatus = AlipayUtil.getInstance().getByte(req.getParameter("trade_status"));
            // 商户订单号
            String out_trade_no = AlipayUtil.getInstance().getByte(req.getParameter("out_trade_no"));
            //支付宝交易号
            String trade_no = AlipayUtil.getInstance().getByte(req.getParameter("trade_no"));
            //付款金额
            String total_amount = AlipayUtil.getInstance().getByte(req.getParameter("total_amount"));
